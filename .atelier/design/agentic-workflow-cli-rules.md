...
approval gate**. The three 2026-08-28 rows are PR #15's, and none of them changed a rule or a
scenario: all three are ticket-scope corrections, which is why the counts above did not move with
them. Rows sharing a date are cited by their subject — the `ctx.git` row, the BR-030 row, the
`--live-checkout` row — rather than by their position. The front matter says so rather than
